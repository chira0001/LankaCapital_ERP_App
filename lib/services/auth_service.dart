...
class AuthService {
  ...
  // Use your computer's local IP address for a physical device
  final String _baseUrl = 'http://10.173.252.234:8080/api/v1'; 
  ...
  Future<bool> login(String username, String password) async {
    try {
      final response = await _dio.post(
        '/auth/login', // Base URL is already set
        data: {
          'username': username,
          'password': password,
        },
  ...
  Future<Map<String, dynamic>?> getCustomerProfile() async {
    try {
      ...
      print("Dio Error fetching profile: ${e.response?.statusCode} - ${e.response?.data}");
    } catch (e) {
  ...
  Future<bool> updateCustomerProfile(String userId, Map<String, dynamic> data) async {
    try {
