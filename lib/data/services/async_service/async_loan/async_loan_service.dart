...
class AsyncLoanService {
  ...
  Future<List<ManageLoanModel>?> manageLoans(int page) async {
    try {
      final Uri url = Uri.parse('$_baseUrl/manage/loan?page=$page');
  ...
  Future<List<LoanUpdate>?> asyncLoans(
    Map<String, dynamic> customersId,
  ) async {
    try {
      final Uri url = Uri.parse('$_baseUrl/async/loans');
      final response = await http
          .post(
            url,
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final data = jsonDecode(response.body);
        if (data is List) {
          return data.map((e) => LoanUpdate.fromServer(e)).toList();
        }
      }
      return null;
    } catch (e) {
